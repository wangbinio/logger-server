...
public class RocketMqConsumerFactory {
    ...
    private final LoggerServerProperties loggerServerProperties;

    /**
     * 创建 RocketMQ 动态消费者工厂。
     *
     * @param rocketMQProperties RocketMQ 配置。
     * @param loggerServerProperties 项目配置。
     */
    public RocketMqConsumerFactory(RocketMQProperties rocketMQProperties,
                                   LoggerServerProperties loggerServerProperties) {
        this.rocketMQProperties = Objects.requireNonNull(rocketMQProperties, "rocketMQProperties 不能为空");
        this.loggerServerProperties = Objects.requireNonNull(loggerServerProperties, "loggerServerProperties 不能为空");
    }
     ...
     * @throws MQClientException 订阅配置异常。
     */
    public DefaultMQPushConsumer createSituationConsumer(String instanceId,
                                                         MessageListenerConcurrently messageListener)
            throws MQClientException {
        return createConsumer(
                buildConsumerGroup("situation", instanceId),
    ...
    private String buildConsumerGroup(String channelType, String instanceId) {
        return loggerServerProperties.getRocketmq().getInstanceConsumerGroupPrefix()
                + "-"
                + channelType
                + "-"
                + sanitize(instanceId);
